...
if (isOpen) {
	...
	if (!surface_exists(shellSurface)) {
		shellSurface = surface_create(1280, 720);
		self._recalculate_shell_properties();
	} else if (surface_get_width(shellSurface) != 1280 || surface_get_height(shellSurface) != 720) {
		surface_resize(shellSurface, 1280, 720);
	}
	...
	if (!surface_exists(scrollSurface)) {
		scrollSurface = surface_create(1280, visibleHeight);
	} else if (surface_get_width(scrollSurface) != 1280 || 
			   surface_get_height(scrollSurface) != visibleHeight) {
		surface_resize(scrollSurface, 1280, visibleHeight);
	}
