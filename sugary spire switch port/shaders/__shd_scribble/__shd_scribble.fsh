...
float SDFValue(vec2 texcoord)
{
    ...
    return (USE_ALPHA_FOR_DISTANCE
             ? sample.a 
             : max(sample.r, max(sample.g, sample.b)))
...
void main()
{
    ...
    else if (u_fFontType == 1.0)
    {
        ...
        if (u_fSecondDraw < 0.5)
        {
            ...
            gl_FragColor.rgb = (gl_FragColor.rgb * gl_FragColor.a +
                                u_vOutlineColour * sample.g * (1.0 - gl_FragColor.a))
                               / outAlpha;
            gl_FragColor.a = outAlpha;

            if (u_vShadowColour.a > 0.0)
            {
                float outAlpha2 = gl_FragColor.a +
                                  u_vShadowColour.a * sample.b * (1.0 - gl_FragColor.a);
                gl_FragColor.rgb = (gl_FragColor.rgb * gl_FragColor.a +
                                    u_vShadowColour.rgb * u_vShadowColour.a * sample.b * (1.0 - gl_FragColor.a))
                                   / outAlpha2;
                gl_FragColor.a = outAlpha2;
            }
        }
    ...
    else
    {
        ...
        if (u_fSecondDraw < 0.5)
        {
            float outlineOffset = u_fOutlineThickness
                * length(fwidth(v_vTexcoord) / u_vTexel)
                / (sqrt(2.0) * u_fSDFRange);
            ...
            if (u_fOutlineThickness > 0.0)
            {
                ...
                gl_FragColor.a = max(
                    ...
                    smoothstep(
                        0.5 - smoothness * spread,
                        0.5 + smoothness * spread,
                        baseDist + outlineOffset
                    )
                );
            }

            if ((u_vShadowColour.a > 0.0)
             && !all(equal(u_vShadowOffsetAndSoftness.xy, vec2(0.0))))
            {
                float alphaShadow = u_vShadowColour.a
                    * smoothstep(
                        0.5 - spread * u_vShadowOffsetAndSoftness.z,
                        0.5 + spread * u_vShadowOffsetAndSoftness.z,
                        SDFValue(v_vTexcoord
                                 - u_vShadowOffsetAndSoftness.xy * fwidth(v_vTexcoord))
                        + outlineOffset
                    );

                float outAlpha3 = gl_FragColor.a
                    + alphaShadow * (1.0 - gl_FragColor.a);
                gl_FragColor.rgb = (gl_FragColor.rgb * gl_FragColor.a
                                    + u_vShadowColour.rgb * alphaShadow * (1.0 - gl_FragColor.a))
                                   / outAlpha3;
                gl_FragColor.a = outAlpha3;
            }
        }
