function strings_read(argument0)
{
...
function dialogue_read(argument0)
{
    var _folders = [];
    var _folder = file_find_first(argument0 + "/*", 16);
    ...
    {
        var _dialogue = file_find_first(_string("{0}/{1}/*", argument0, _folders[i]), 0);
        
        while (_dialogue != "")
        {
            ds_map_add(obj_pause.dialoguemap, _string("{0}/{1}", _folders[i], string_replace(_dialogue, ".psn", "")), parse_dialogue(_string("{0}/{1}/{2}", argument0, _folders[i], _dialogue)));
...
function string_get(argument0)
{
    ...
    for (var p = 1; p < argument_count; p++)
    {
        ...
        _replace = _isdirectory ? _aroot : _string(_replace);
        
        if (is_array(_prevvar))
        {
            for (var i = 0; i < array_length(_prevvar); i++)
                _prevvar[i] = string_replace(_prevvar[i], _string("{{0}}", p - 1), _replace);
        }
        else
        {
            _prevvar = string_replace(_prevvar, _string("{{0}}", p - 1), _replace);
        }
    }
...
function dialogue_get(argument0)
{
...
function parse_dialogue(argument0 = -1)
{
    var _str = "";
