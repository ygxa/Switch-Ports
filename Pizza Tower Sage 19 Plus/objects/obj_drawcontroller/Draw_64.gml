if (instance_exists(obj_player))
{
    ...
    if (!scr_hudroomcheck() && obj_player.state != (33 << 0))
    {
            ...
            if (!global.timetrial)
            {
                ...
                for (var i = 0; i < string_length(_scstr); i++)
                {
                    ...
                    if (_sc != prevcollect)
                    {
                        ...
                        if (prevcollect != -1 && string_char_at(_scstr, i + 1) != string_char_at(_string(prevcollect), i + 1))
                        {
                            collectcolor[i] = irandom_range(1, 4);
                            scorey[i] = 0;
                            scorebounce[i] = -4;
                    ...
                    draw_sprite_ext(spr_font_collectshadow, digit, collectHUDX + (40 * (i - (string_length(_string(_sc)) / 2))) + shakex, yshiftl + numberY + shakey, 1, 1, 0, c_white, alpha);
                    draw_sprite_ext(spr_font_collect, digit, collectHUDX + (40 * (i - (string_length(_string(_sc)) / 2))) + shakex, yshiftl + numberY + shakey + floor(scorey[i]), 1, 1, 0, c_white, alpha);
                    shader_set(shd_premultiply);
                }
        ...
        if (!_tutorial)
        {
            if (global.currentbadge == (3 << 0))
            {
                ...
                if (_frac > 0)
                    _text = _string("{0}.{1}{2}", _text, _frac, _f);
                else
                    _text = _string("{0}{1}", _text, _f);
            ...
            else
            {
                ...
                var shakeyshakey = comboshake * 3;
                var i = string_length(_string(visualcombo));
        ...
        if (!_tutorial)
        {
            if (global.panic || global.miniboss)
            {
                ...
                var s = _string(ceil(global.escapetime) % 60);
                
                if (s < 10)
                    s = "0" + s;
                
                __draw_text_colour_hook(random_range(1, -1) + 480, random_range(1, -1) + get_game_height() + escapetimery + (30 * isnewhud), _string("{0}:{1}", m, s), c, c, c, c, alpha);
            }
        }
    }
    ...
    if (!asset_has_tags(room, "No Save Timer", 3))
    {
        if (global.timertype == 2 || global.timertype == 3)
            _string += _string("{0}:{1}:{2}.{3}", _filehours, _fileminutes, _fileseconds, _filemilliseconds);
        ...
        if ((global.timertype == 1 && (!asset_has_tags(room, "No Timer", 3) || (room == rank_room || room == rank_roomtutorial))) || global.timertype == 3)
            _string += _string("{0}:{1}:{2}.{3}", _hours, _minutes, _seconds, _milliseconds);
    }
