function scr_loadgame()
{
    ...
    if (file_exists("Save.sav"))
    {
        ...
        for (i = 0; i < array_length_1d(global.ruins_flag); i++)
            global.ruins_flag[i] = ini_read_real("RuinsFlags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.flag); i++)
            global.flag[i] = ini_read_real("Flags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.snowdin_flag); i++)
            global.snowdin_flag[i] = ini_read_real("SnowdinFlags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.dunes_flag); i++)
            global.dunes_flag[i] = ini_read_real("DunesFlags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.dunes_flag_ext); i++)
            global.dunes_flag_ext[i] = ini_read_real("DunesFlagsExt", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.sworks_flag); i++)
            global.sworks_flag[i] = ini_read_real("SworksFlags", __string(i), false);
        ...
        {
            global.sworks_robot_sprite[i] = ini_read_real("RbSprite", __string(i), 0);
            global.sworks_robot_x[i] = ini_read_real("RbX", __string(i), 0);
            global.sworks_robot_y[i] = ini_read_real("RbY", __string(i), 0);
            global.sworks_robot_depth[i] = ini_read_real("RbD", __string(i), 0);
            global.sworks_robot_scale[i] = ini_read_real("RbS", __string(i), 0);
            global.sworks_robot_angle[i] = ini_read_real("RbA", __string(i), 0);
        }
        
        ds_grid_read(global.sworks_id_grid, ini_read_string("SworksFlags", "sworks_id", ""));
        
        for (i = 0; i < array_length_1d(global.item_stock); i++)
            global.item_stock[i] = ini_read_real("ItemStock", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.hotland_flag); i++)
            global.hotland_flag[i] = ini_read_real("HotlandFlags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.flowey_flag); i++)
            global.flowey_flag[i] = ini_read_real("FloweyFlags", __string(i), false);
        ...
        for (i = 0; i < array_length_1d(global.extra_flag); i++)
            global.extra_flag[i] = ini_read_real("ExtraFlags", __string(i), false);
        
        for (i = 0; i < array_length_1d(global.mail_flag); i++)
            global.mail_flag[i] = ini_read_real("MailFlags", __string(i), false);
        ...
        for (i = 0; i < array_length(global.fun_event); i++)
            global.fun_event[i] = ini_read_real("Fun Events", __string(i), 0);
        
        for (i = 1; i <= 8; i++)
            global.item_slot[i] = ini_read_string("Items", "0" + __string(i - 1), "Nothing");
        ...
        for (i = 1; i < array_length_1d(global.kill_number); i++)
        {
            ...
            for (j = 0; j < array_length_2d(global.kill_area, i); j++)
                global.kill_area[i][j] = ini_read_real("Kill Area", __string(i) + __string(j), 5);
        }
