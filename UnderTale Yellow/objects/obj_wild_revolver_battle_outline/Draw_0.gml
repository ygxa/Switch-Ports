var target_pos, x_offset, y_offset;

if (live_call())
    return global.live_result;

draw_sprite_ext(sprite_index, image_index, x, y, image_xscale, image_yscale, 0, c_white, image_alpha);
target_pos = target_angle + 90;

for (i = 0; i < target_max; i++)
{
    if ((target_pos - ((i * 360) / target_max)) >= -(360 / target_max))
    {
        x_offset = target_radius * image_xscale * cos(degtorad(target_pos - ((i * 360) / target_max)));
        y_offset = target_radius * image_xscale * sin(degtorad(target_pos - ((i * 360) / target_max)));
