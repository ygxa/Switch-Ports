function player_faker_CPUfollow()
{
    ...
    if (!cpu_recovering)
    {
        ...
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 80 && vsp == 0 && (place_meeting(x, y + 1, ObjWall) || detect == 1) && grounded && cputimerbaybee == 0)
        {
            CPUtimetillpath = 200;
        ...
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 250 && o_cpu_findme.y < (y + 20) && o_cpu_findme.y > (y - 20) && cputimerbaybee == 0 && global.CPULVL > 1 && damagemeter == damagemetermax)
        {
            state = (25 << 0);
            CPUtimetillpath = 200;
        }
        
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 300 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 200 && o_cpu_findme.y < (y + 20) && o_cpu_findme.y > (y - 20) && cputimerbaybee == 0 && global.CPULVL == 5 && damagemeter != damagemetermax && noisebomb < 1)
        {
            state = (20 << 0);
            noisebomb = 30;
            CPUtimetillpath = 200;
