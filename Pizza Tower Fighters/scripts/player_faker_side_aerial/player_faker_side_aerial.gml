function player_faker_side_aerial()
{
    ...
    if (sprite_index != s_fakepep_sideaerial)
    {
        image_speed = 1;
        sprite_index = s_fakepep_sideaerial;
        image_index = 0;
        ds_list_clear(HitByAttack);
    }
    
    mask_index = s_fakepep_sideaerial_col;
    
    if (floor(image_index) == 5 && !audio_is_playing(su_pep_punch))
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                    player_hurt(12, 2, other.facing, 5);
            }
        }
    }
    ...
    if (place_meeting(x, y + 1, ObjWall) || floor(image_index) == (image_number - 1) || place_meeting(x, y + 1, o_slope))
        state = (0 << 0);
}
