function __scribble_class_page() constructor
{
    ...
    static __submit = function(argument0, argument1)
    {
        ...
        repeat (array_length(__vertex_buffer_array))
        {
            ...
            {
                shader_set_uniform_f(_msdf_u_vTexel, _data[(4 << 0)], _data[(5 << 0)]);
                shader_set_uniform_f(_msdf_u_fMSDFRange, argument0 * _data[(2 << 0)]);
                shader_set_uniform_f(_msdf_u_fMSDFThicknessOffset, __scribble_state.__msdf_thickness_offset + _data[(3 << 0)]);
    ...
    static __get_glyph_data = function(argument0)
    {
        ...
        if (argument0 < 0)
        {
            return 
            {
                ...
                bottom: ds_grid_get(__glyph_grid, 0, (2 << 0))
            };
        }
        else
        {
            ...
            return 
            {
                unicode: ds_grid_get(__glyph_grid, argument0, (0 << 0)),
                left: ds_grid_get(__glyph_grid, argument0, (1 << 0)),
                top: ds_grid_get(__glyph_grid, argument0, (2 << 0)),
                right: ds_grid_get(__glyph_grid, argument0, (3 << 0)),
                bottom: ds_grid_get(__glyph_grid, argument0, (4 << 0))
            };
        }
    };
    ...
    static __get_vertex_buffer = function(argument0, argument1, argument2, argument3, argument4)
    {
        ...
        if (_data == undefined)
        {
            ...
            {
                argument4.__uses_standard_font = true;
                _shader = __shd_scribble;
            }
            else
            {
                argument4.__uses_msdf_font = true;
            ...
            _data[(2 << 0)] = argument1;
            _data[(3 << 0)] = argument2;
            _data[(4 << 0)] = texture_get_texel_width(argument0);
            _data[(5 << 0)] = texture_get_texel_height(argument0);
            _data[(6 << 0)] = _shader;
            _data[(8 << 0)] = argument3;
