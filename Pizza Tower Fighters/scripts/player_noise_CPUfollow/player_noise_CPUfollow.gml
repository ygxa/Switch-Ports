function player_noise_CPUfollow()
{
    ...
    if (!cpu_recovering)
    {
        ...
        if ((!grounded && o_cpu_findme.y > (y + 10) && x < (o_cpu_findme.x + 58) && x > (o_cpu_findme.x - 58)) && cputimerbaybee == 0 && global.CPULVL > 1 && !noisedownaerialblock)
        {
            state = (4 << 0);
            alarm[0] = 50;
            noisedownaerialblock = true;
            CPUtimetillpath = 200;
        }
        ...
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 200 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 100 && (place_meeting(x, y + 1, ObjWall) || detect == 1) && vsp == 0 && y > (o_cpu_findme.y - 12) && y < (o_cpu_findme.y + 12) && grounded && cputimerbaybee == 0 && !instance_exists(o_player_noise_explosion))
        {
            state = (3 << 0);
            CPUtimetillpath = 200;
        }
        
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 400 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 140 && (place_meeting(x, y + 1, ObjWall) || detect == 1) && vsp == 0 && y > (o_cpu_findme.y - 20) && y < (o_cpu_findme.y + 12) && grounded && cputimerbaybee == 0 && noisebomb < 1 && global.CPULVL == 4)
        {
            state = (10 << 0);
            CPUtimetillpath = 200;
        }
        
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 300 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 100 && y > (o_cpu_findme.y - 20) && y < (o_cpu_findme.y + 12) && cputimerbaybee == 0 && noisebomb < 1 && global.CPULVL == 5)
        {
            state = (10 << 0);
            CPUtimetillpath = 200;
        }
        
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 300 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 100 && y > (o_cpu_findme.y - 20) && y < (o_cpu_findme.y + 12) && cputimerbaybee == 0 && damagemeter == damagemetermax && global.CPULVL > 2)
        {
            state = (22 << 0);
            CPUtimetillpath = 200;
        }
        
        if (point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) < 100 && point_distance(x, y, o_cpu_findme.x, o_cpu_findme.y) > 60 && (place_meeting(x, y + 1, ObjWall) || detect == 1) && vsp == 0 && y > (o_cpu_findme.y - 50) && y < (o_cpu_findme.y + 12) && grounded && cputimerbaybee == 0 && noisebomb < 1 && global.CPULVL == 5)
        {
            CPUtimetillpath = 200;
            state = (9 << 0);
        }
