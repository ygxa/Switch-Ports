function player_test_dash_attack()
{
    if (sprite_index != s_test_dash_attack)
    {
        audio_stop_sound(su_pep_dash);
        audio_play_sound(su_pep_dashattack, 50, false);
        sprite_index = s_test_dash_attack;
        image_index = 0;
        image_speed = 1;
    ...
    vsp += grv;
    player_col();
    mask_index = s_test_dash_attackcol;
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                    player_hurt(12, 6, other.facing, 6);
            }
        }
    }
