function player_doise_up_aerial()
{
    ...
    if (sprite_index != s_noise_upAerial)
    {
        image_speed = 0.9;
        audio_play_sound(su_noise_spin, 50, false);
        sprite_index = s_noise_upAerial;
        image_index = 0;
        ds_list_clear(HitByAttack);
    }
    
    mask_index = s_noise_upAerial_col;
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                    player_hurt(10, 7, other.facing, 5);
            }
        }
    }
