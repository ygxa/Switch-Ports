function player_noise_down_Aerial()
{
    if (sprite_index != s_noise_downaerial)
    {
        image_speed = 0.6;
        sprite_index = s_noise_downaerial;
        image_index = 0;
    ...
    y += vsp;
    mask_index = s_noise_downaerial_col;
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                {
                    audio_play_sound(su_noise_panhit, 50, false);
                    player_hurt(2, -2, -other.facing, 5);
                }
            }
        }
    }
