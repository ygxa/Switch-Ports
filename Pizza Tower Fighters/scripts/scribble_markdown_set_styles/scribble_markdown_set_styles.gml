function scribble_markdown_set_styles(argument0, argument1 = false)
{
    ...
    if (!argument1)
    {
        if (!is_struct(argument0))
            __scribble_error("Input was not a struct (datatype=", typeof(argument0), ")");
        
        var _root_names_array = variable_struct_get_names(argument0);
        var _i = 0;
        ...
        repeat (array_length(_root_names_array))
        {
            ...
            {
                var _value = variable_struct_get(argument0, _root_name);
    ...
    _scribble_state.__markdown_styles_struct = argument0;
    return true;
}
