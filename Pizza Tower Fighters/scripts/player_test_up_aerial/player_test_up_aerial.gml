function player_test_up_aerial()
{
    ...
    if (sprite_index != s_test_up_aerial)
    {
        image_speed = 1;
        sprite_index = s_test_up_aerial;
        image_index = 0;
        ds_list_clear(HitByAttack);
    }
    
    mask_index = s_test_up_aerialcol;
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                    player_hurt(5, 10, other.facing, 2);
            }
        }
    }
