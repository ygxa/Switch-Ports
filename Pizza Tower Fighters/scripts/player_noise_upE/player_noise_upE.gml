function player_noise_upE()
{
    hsp = 0;
    vsp = 0;
    
    if (sprite_index != s_noise_upE)
    {
        image_speed = 2;
        sprite_index = s_noise_upE;
        image_index = 0;
        ds_list_clear(HitByAttack);
    }
    
    if (floor(image_index) == 8 && !audio_is_playing(su_noise_glovehit))
    {
        image_speed = 1.5;
    ...
    if (Hits > 0)
    {
        for (var i = 0; i < Hits; i++)
        {
            ...
            if (ds_list_find_index(HitByAttack, HitID) == -1)
            {
                ...
                with (HitID)
                    player_hurt(10, 12, other.facing, 4);
            }
        }
    }
