...
with (obj_player1)
{
    if ((state == 100 && place_meeting(x, y, obj_startgate)) || state == 91)
        can_pause = false;
}

if (input_check_pressed("start") && global.shellactivate == false && can_pause)
{
    if (room == rm_mainmenu && state == 1)
    {
        if (obj_mainmenu.state != 1)
            exit;
    ...
    if (state == 1)
    {
        ...
        if (!surface_exists(pause_surf))
            pause_surf = surface_create(global.resolutions[0][1][0], global.resolutions[0][1][1]);
        ...
        if (surface_exists(pause_surf))
        {
            ...
            draw_surface_stretched_ext(application_surface, 0, 0, global.resolutions[0][1][0], global.resolutions[0][1][1], 16777215, 1);
        ...
        pause_spr = sprite_create_from_surface(pause_surf, 0, 0, global.resolutions[0][1][0], global.resolutions[0][1][1], 0, 0, 0, 0);
...
if (state == 0 && !instance_exists(obj_option) && !instance_exists(obj_optionNew))
{
