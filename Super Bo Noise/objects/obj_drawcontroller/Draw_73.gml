if (kidsparty_lightning || dark_lightning)
{
    ...
    if kidsparty_lightning
    {
        if surface_exists(surf)
        {
            ...
            with (obj_patrolcone)
            {
                if (instance_exists(baddieID) && baddieID.state != (140 << 0) && baddieID.state != (6 << 0) && baddieID.state != (139 << 0) && baddieID.state != (82 << 0))
                {
                    var points = self.get_triangle_points(x, y, image_angle, len, size)
                    var c = 16777215
        ...
        if surface_exists(surf2)
        {
            ...
            with (obj_patrolcone)
            {
                if (instance_exists(baddieID) && baddieID.state != (140 << 0) && baddieID.state != (6 << 0) && baddieID.state != (139 << 0) && baddieID.state != (82 << 0))
                {
                    ...
                    {
                        points = self.get_triangle_points(x, y, image_angle, len, size)
                        c = 255
                        draw_triangle_color((x - surf_x), (y - surf_y), (points[0] - surf_x), (points[1] - surf_y), (points[2] - surf_x), (points[3] - surf_y), c, c, c, 0)
                    }
                }
