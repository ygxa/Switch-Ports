...
ds_map_set(player_hurtstates, 110, 20);
ds_map_set(player_hurtstates, 82, 20);
ds_map_set(player_hurtstates, 7, 20);
ds_map_set(player_hurtstates, 99, 20);
ds_map_set(boss_hurtstates, 85, 60);
ds_map_set(boss_hurtstates, 155, 60);
ds_map_set(boss_hurtstates, 159, 90);
ds_map_set(boss_hurtstates, 78, 200);
ds_map_set(boss_hurtstates, 82, 30);
ds_map_set(boss_hurtstates, 110, 70);
...
{
    if (argument1.state != 86 || argument1.parry_inst == -4)
    {
        var _prevstate = state;
        SUPER_player_hurt(argument0, argument1);
        
        if (_prevstate == 155 || _prevstate == 159 || _prevstate == 85 || _prevstate == 78)
        {
            with (obj_camera)
            {
                shake_mag = 3;
    ...
    else if (state == 78)
    {
        with (argument1)
        {
            if (state == 139 || state == 63)
            {
                x = hitX;
                y = hitY;
            }
            
            if (other.state == 139 || other.state == 63)
            {
                other.x = hitX;
                other.y = hitY;
            }
