...
if (instance_exists(player) && player.state != 66 && player.state != 91 && !follow_golf && !detach)
{
    var target = player;
    var ty = target.y;
    var tx = target.x;
    ...
    else if (player.state == 106 || player.state == 123)
    {
        var _targetcharge = player.xscale * ((player.movespeed / 4) * 50);
        var _tspeed = 0.3;
        chargecamera = approach(chargecamera, _targetcharge, _tspeed);
    }
    else if ((abs(player.hsp) >= 16 || (player.state == 63 && player.tauntstoredmovespeed >= 16)) && player.state != 39 && player.state != 99)
    {
