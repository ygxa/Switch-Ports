...
if (state == 136)
{
    if (!chasing)
    {
        ...
        if (targetplayer.x > (x - 1900) && targetplayer.x < (x + 1900) && y <= (targetplayer.y + global.resolutions[0][1][1]) && y >= (targetplayer.y - global.resolutions[0][1][1]))
        {
            chasing = 1;
...
else if (state == 143)
{
    ...
    if ((inst_front != -4 || inst_up != -4 || (inst_down == -4 && inst_down2 == -4)) && targetplayer.y <= (y + 32) && grounded && state != 130)
    {
        vsp = -11;
...
if (sprite_index == scaredspr && state == 128)
    invincible = 0;
