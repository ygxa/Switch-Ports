if (text_state == (20 << 0) && text_y <= (-((text_sprite_height * text_yscale))))
    return;
...
for (var i = 0; i < array_length(levelarray); i++)
{
    ...
    if b.toppins
    {
        ...
        {
            draw_sprite(spr_list_star, b.toppinarr[j], (xx + x2), y1)
            if b.toppinarr[j]
                toppinseen = true
            x2 += w
        }
    ...
    if b.secrets
    {
        ...
        w = sprite_get_width(spr_list_bubble)
        x2 = 3 * w / 2
        x2 *= -1
        x2 += (w / 2)
        for (j = 0; j < 3; j++)
        {
            draw_sprite(spr_list_bubble, (j + 1) <= b.secretcount, (xx + x2), y1)
            x2 += w
        }
    ...
    if b.treasure
        draw_sprite(spr_list_treasure, b.gottreasure, (xx + xs), y1)
    xx += level_xpad
}
