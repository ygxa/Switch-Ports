function __scribble_gen_7_build_pages()
{
    ...
    _page_data.__glyph_start = ds_grid_get(_word_grid, ds_grid_get(_line_grid, 0, 2), 2);
    ...
    repeat (_line_count)
    {
        var _line_height = ds_grid_get(_line_grid, _i, 5);
        var _starts_manual_page = ds_grid_get(_line_grid, _i, 7);
        
        if (!_starts_manual_page && ((_line_y + _line_height) < _simulated_model_height || _page_start_line >= _i))
        {
            ds_grid_set(_line_grid, _i, 1, _line_y);
        ...
        else
        {
            ...
            _page_data.__glyph_end = ds_grid_get(_word_grid, ds_grid_get(_line_grid, _page_end_line, 3), 3);
            _page_data.__glyph_count = (1 + _page_data.__glyph_end) - _page_data.__glyph_start;
            _page_data.__width = ds_grid_get_max(_line_grid, _page_start_line, 4, _page_end_line, 4);
            ...
            _page_anim_start = ds_grid_get(_glyph_grid, _page_data.__glyph_start, 19);
            _page_anim_end = ds_grid_get(_glyph_grid, _page_data.__glyph_end, 19);
            ...
            if (_randomize_animation)
            {
                ...
                repeat (_page_data.__character_count)
                {
                    ds_grid_set(_glyph_grid, _glyph_start + _i, 19, _animation_randomize_array[_i]);
                    _i++;
                }
            }
            else
            {
                ds_grid_add_region(_glyph_grid, _page_data.__glyph_start, 19, _page_data.__glyph_end, 19, -_page_anim_start);
            }
            
            _page_data = __new_page();
            _page_data.__line_start = _i;
            _page_data.__glyph_start = ds_grid_get(_word_grid, ds_grid_get(_line_grid, _i, 2), 2);
            _page_start_line = _i;
            ds_grid_set(_line_grid, _i, 1, 0);
    ...
    var _page_anim_start = ds_grid_get(_glyph_grid, _page_data.__glyph_start, 19);
    var _page_anim_end = ds_grid_get(_glyph_grid, _page_data.__glyph_end, 19);
    _page_data.__character_count = (1 + _page_anim_end) - _page_anim_start;
