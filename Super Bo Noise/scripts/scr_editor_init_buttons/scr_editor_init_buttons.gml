function scr_editor_init_buttons() //scr_editor_init_buttons
{
    with (instance_create_depth(0, 0, -100, obj_button))
    {
        ...
        OnSelect = function() //anon@170@scr_editor_init_buttons_scr_editor_init_buttons
        {
            var a = [["SAVE", -4, -4], ["LOAD LEVEL", -4, -4], ["EXIT EDITOR", -4, -4]]
            create_buttons_array(0, 64, 32, 24, a)
        }

        OnDeselect = function() //anon@332@scr_editor_init_buttons_scr_editor_init_buttons
        {
            with (obj_button)
            {
                if (parent == other.id)
    ...
    with (instance_create_depth(128, 0, -100, obj_button))
    {
        ...
        selectedid = 0
        OnSelect = function() //anon@724@scr_editor_init_buttons_scr_editor_init_buttons
        {
            onselect = function() //anon@755@anon@724_scr_editor_init_buttons_scr_editor_init_buttons
            {
                with (parent)
        ...
        OnDeselect = function() //anon@1617@scr_editor_init_buttons_scr_editor_init_buttons
        {
            with (obj_button)
            {
                if (parent == other.id)
