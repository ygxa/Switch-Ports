function scribble_super_glyph_copy(argument0, argument1, argument2)
{
    ...
    ds_grid_set_region(_target_glyph_data_grid, 0, 7, ds_grid_width(_target_glyph_data_grid), 7, max(_target_font_data.__height, _source_font_data.__height));
}

function __scribble_super_glyph_copy_common(argument0, argument1)
{
    if (argument1.__sdf == undefined)
    {
        __scribble_error("Cannot determine if the source font is an SDF font. Please add glyphs to it");
    }
    else if (argument0.__sdf == undefined)
    {
    }
    else if (argument0.__sdf || argument1.__sdf)
    {
        if (argument0.__sdf == false)
            __scribble_error("Cannot mix standard/sprite fonts with SDF fonts (target is not an SDF font)");
        
        if (argument1.__sdf == false)
            __scribble_error("Cannot mix standard/sprite fonts with SDF fonts (source is not an SDF font)");
        
        if (argument1.__sdf_pxrange == undefined)
            __scribble_error("Source font's SDF pxrange must be defined before copying glyphs");
        
        if (argument0.__sdf_pxrange != undefined && argument0.__sdf_pxrange != argument1.__sdf_pxrange)
            __scribble_error("SDF font pxrange must match (target = ", argument0.__sdf_pxrange, " vs. source = ", argument1.__sdf_pxrange, ")");
    }
    
    argument0.__sdf = argument1.__sdf;
    argument0.__sdf_pxrange = argument1.__sdf_pxrange;
}
...
function __scribble_glyph_duplicate(argument0, argument1, argument2, argument3, argument4, argument5)
{
    ...
    if (_target_x == undefined)
    {
        ...
        ds_grid_resize(argument3, _target_x + 1, 19);
    }
    else if (!argument5)
    {
        exit;
    ...
    ds_grid_set_grid_region(argument3, argument1, _source_x, 0, _source_x, 19, _target_x, 0);
}
