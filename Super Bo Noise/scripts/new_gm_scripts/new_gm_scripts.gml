...
function string_trim(s) {
    var i = 1, j = string_length(s);
    while (i <= j && ord(string_char_at(s, i)) <= 32) i++;
    while (j >= i && ord(string_char_at(s, j)) <= 32) j--;
    return string_copy(s, i, j - i + 1);
}

function string_ends_with(s, ending) {
    var sl = string_length(s), el = string_length(ending);
    if (el > sl) return false;
    return string_copy(s, sl - el + 1, el) == ending;
}
