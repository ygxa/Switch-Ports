function scr_gpinput_isaxis(argument0) //scr_gpinput_isaxis
{
...
function scr_input_stickpressed(argument0) //scr_input_stickpressed
{
    var s = string(argument0)
    return ds_map_find_value(global.stickpressed, s);
}

function scr_input_stickpressed_update(argument0, argument1) //scr_input_stickpressed_update
{
    if (argument0 == undefined)
        argument0 = global.player_input_device
    if (argument1 == undefined)
        argument1 = global.deadzones[(0 << 0)]
...
function scr_checkdeadzone(argument0, argument1) //scr_checkdeadzone
{
    ...
    if instance_exists(obj_player1)
    {
        ...
        {
            case "upC":
                if (obj_player1.state == (101 << 0))
                    dz = global.deadzones[(4 << 0)]
                break
            case "downC":
                if (obj_player1.state == (102 << 0))
...
function scr_check_joysticks(argument0, argument1) //scr_check_joysticks
{
    if (argument1 == undefined)
        argument1 = 0.5
    var sticks = [32785, 32786, 32787, 32788]
...
function scr_checksuperjump() //scr_checksuperjump
{
...
function scr_checkgroundpound() //scr_checkgroundpound
{
...
{
    if (keyboard_check_pressed(vk_return) || input_get("jump").pressed)
        return -1;
...
function scr_input_icon_define() //scr_input_icon_define
{
...
function scr_input_set_icon(argument0) //scr_input_set_icon
{
    var ico = [spr_null, 0]
    switch argument0
    {
...
function input_addIcon(argument0, argument1, argument2) //input_addIcon
{
...
function scr_input_get_icon(argument0, argument1) //scr_input_get_icon
{
    if (argument1 == undefined)
        argument1 = false
    var result = []
