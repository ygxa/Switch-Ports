function scribble_glyph_set(argument0, argument1, argument2, argument3, argument4 = false)
{
    ...
    if (argument1 == -3 || argument1 == "all")
    {
        ...
        if (argument2 == 8)
            ds_grid_set(_grid, _glyph_index, 5, ds_grid_get(_grid, _glyph_index, 8));
        
        if (argument2 == 5)
            ds_grid_set(_grid, _glyph_index, 8, ds_grid_get(_grid, _glyph_index, 5));
        
        if (argument2 == 6)
            ds_grid_set(_grid, _glyph_index, 7, ds_grid_get(_grid, _glyph_index, 6));
        
        if (argument2 == 7)
            ds_grid_set(_grid, _glyph_index, 6, ds_grid_get(_grid, _glyph_index, 7));
        
        if (argument2 == 6 || argument2 == 7)
        {
            ds_grid_set_region(_grid, 0, 7, ds_grid_width(_grid) - 1, 7, ds_grid_get(_grid, _glyph_index, 7));
    ...
    else
    {
        ...
        {
            if (argument2 == 8)
                ds_grid_set(_grid, _glyph_index, 5, _new_value);
            
            if (argument2 == 5)
                ds_grid_set(_grid, _glyph_index, 8, _new_value);
            
            if (argument2 == 6)
                ds_grid_set(_grid, _glyph_index, 7, _new_value);
            
            if (argument2 == 7)
                ds_grid_set(_grid, _glyph_index, 6, _new_value);
            
            if (argument2 == 6 || argument2 == 7)
            {
                ds_grid_set_region(_grid, 0, 7, ds_grid_width(_grid) - 1, 7, _new_value);
                _font_data.__calculate_font_height();
            }
        }
