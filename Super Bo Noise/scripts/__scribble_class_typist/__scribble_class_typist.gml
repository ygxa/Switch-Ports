            ...
            if (!_paused)
            {
                ...
                while (_remaining > 0)
                {
                    ...
                    if (_head_pos >= __last_character)
                    {
                        ...
                        {
                            var _glyph_ord = ds_grid_get(_page_data.__glyph_grid, __last_character - 1, 0);
                            var _delay = variable_struct_get(__character_delay_dict, _glyph_ord);
                            _delay = (_delay == undefined) ? 0 : _delay;
                            
                            if (__last_character > 1)
                            {
                                _glyph_ord = (_glyph_ord << 32) | ds_grid_get(_page_data.__glyph_grid, __last_character - 2, 0);
