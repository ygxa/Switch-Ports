...
if animation_end() && sprite_index == spr_pizzano_hudpress sprite_index = spr_pizzano_hud
switch state {
	case states.normal: break;
	case states.Throw:
		if animation_end(,4) {
			var cur_trap = instance_nearest(trap_x, trap_y, obj_pizzanotrap_parent)
			//show_debug_message(cur_trap)
			//if cur_trap < 0 cur_trap = instance_nearest(obj_player.x, obj_player.y, obj_pizzanotrap_parent)
			with cur_trap {
				event_user(0)	
