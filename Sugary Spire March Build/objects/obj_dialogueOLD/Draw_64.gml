...
for (var i = 0; i < floor(drawchar); i++) {
		var letter = [
			string_char_at(_text, i),
			string_char_at(_text, i + 1),
			string_char_at(_text, i + 2),	
			string_char_at(_text, i + 3),	
			string_char_at(_text, i + 4),	
			string_char_at(_text, i + 5),	
			string_char_at(_text, i + 6),	
			string_char_at(_text, i + 7),	
			string_char_at(_text, i + 8),	
		]
...
//draw_roundrect_color_ext(100, 400 + yoffset, 860, 500 + yoffset, 16, 16, c_black, c_black, 0)




//draw_text_ext(_textX, 415 + yoffset, textDraw, 22, _wrapX)
