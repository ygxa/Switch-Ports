...
if (sprite_index != spr_player_superjumpprep)
	...
	crouchslideAnim = 1;
	crouchAnim = 1;
	if (floor(image_index) == (image_number - 1) && sprite_index == spr_player_superjumpprep) sprite_index = spr_player_superjumppreplight;
	if ((!key_up) && (grounded || Sjumpcan_doublejump == true) && (sprite_index == spr_player_superjumppreplight || sprite_index == spr_player_superjumpleft || sprite_index == spr_player_superjumpright || sprite_index = spr_pizzano_sjumpprep) && (!scr_solid(x, (y - 16))) && (!scr_solid(x, (y - 32)))) {
	    ...
	    sprite_index = spr_player_superjump; if character = "N" sprite_index = spr_pizzano_sjump
	    state = states.Sjump;
	    vsp = -12;
		movespeed = 12;
		Sjumpcan_doublejump = false;
	}
	if (!audio_is_playing(sound_superjumpcharge2)) scr_sound(sound_superjumpcharge2);
	image_speed = 0.35;
}