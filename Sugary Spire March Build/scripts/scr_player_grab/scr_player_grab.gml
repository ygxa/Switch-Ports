function state_player_grab() {
	...
	if grounded {
	    ...
	    if ((!place_meeting(x, (y + 1), obj_railh)) && (!place_meeting(x, (y + 1), obj_railh2)))
	        ...
	        hsp = ((move * movespeed) + 5);
	    if (move != 0) {
	        if (movespeed < 6) movespeed += 0.5;
	        else if (floor(movespeed) == 6) movespeed = 6;
	    }
	    else movespeed = 0;
	    if (movespeed > 6) movespeed -= 0.1;
	    if (move != 0 && sprite_index != spr_player_swingding) xscale = move;
	    if (move != 0)
	    {
	        if (movespeed < 3 && move != 0) image_speed = 0.35;
	        else if (movespeed > 3 && movespeed < 6) image_speed = 0.45;
	...
	if (swingdingbuffer > 0) swingdingbuffer -= 1;
	...
	if ((!instance_exists(obj_cloudeffect)) && grounded && move != 0 && (floor(image_index) == 4 || floor(image_index) == 10)) instance_create(x, (y + 43), obj_cloudeffect);
	if (key_down && grounded) {
	...
	if (move != 0 && (floor(image_index) == 3 || floor(image_index) == 8) && steppy == 0) steppy = 1;
	if (move != 0 && floor(image_index) != 3 && floor(image_index) != 8) steppy = 0;
	if (sprite_index != spr_player_swingding) image_speed = 0.35;
	else image_speed = (swingdingbuffer / 600)
}