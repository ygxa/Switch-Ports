function scr_enemy_throw() {
	...
	if (object_index = obj_fizz || object_index == obj_conedrone) vsp = 0
	...
	if ((bombreset <= 0) && (floor(image_index) >= throw_frame)) {
		...
		switch object_index
		{
			...
			case obj_flyingguard:
				...
				if object_index != obj_flyingguard vsp = -9
				alarmed = true
			...
			case obj_crackerkicker:
				if (!audio_is_playing(sound_enemythrow))
					scr_sound(sound_enemythrow);
				instance_create(x + (image_xscale * 50), y, obj_crackerkicker_kickhitbox, {image_xscale : image_xscale, baddieID : id});
				bombreset = 200;
	            break	
			}
		}
